# Ticket 08 验收：对拍链回归 + 转换计时复测（一次性脚本，闭票后删除）
#
# 与 accept-07/accept_chain.py 同口径：转换 AHT → compare_two_mdf vs
# 20260817 基线（230 组逐位一致）+ full_compare vs CANoe 参考（70/70 组、
# 3434 处已知真实差异 = 基线同分）+ 转换总耗时对比本机基线 29.8s
# （H1+H2a 后，master plan §5.2；H3 不在转换路径，总耗时应维持同数）。
#
# 运行：& C:\ProgramData\anaconda3\python.exe .scratch/closeout/accept-08/accept_chain.py
import subprocess
import sys
import time
...
def main():
    ...
    print(f"转换总耗时 {dt:.2f}s（本机基线 29.8s，H1+H2a 后） warnings={result.warnings}")
    ...
    r = subprocess.run([PY, "tools/full_compare.py", str(OUT), str(CANOE),
                        ...
                        "--outdir", str(ROOT / ".scratch/closeout/accept-08")])
