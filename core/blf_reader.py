...
def _iter_frames(path: str, progress_cb=None, cancel_cb=None) -> Iterator[Frame]:
    ...
    def _walk(data: bytes) -> tuple[list[Frame], bytes]:
        """容器内对象行走（python-can _parse_data 语义），返回 (帧, 尾部)。

        尾部 = 容器末尾不足一个对象头 / 跨容器对象的剩余字节，留给下一
        容器衔接（_parse_container 的 _tail 语义）。
        """
        max_pos = len(data)
        pos = 0
        frames = []
    ...
    # 文件头：SYSTEMTIME（毫秒精度，唯一可靠的绝对基准——A19G1 实测
    # measurement_start_time 字段存的是 10ms 级残值，不能当绝对基准用）
    with open(path, "rb") as f:
        header = FILE_HEADER_STRUCT.unpack(f.read(FILE_HEADER_STRUCT.size))
        if header[0] != b"LOGG":
            raise BLFParseError("Unexpected file format")
        start_ns = _systemtime_ns(header)
        f.seek(header[1])  # 跳过文件头剩余
        total = os.path.getsize(path)
        last = -1
        tail = b""
