...
日期：2026-08-14
状态：**分析完成（全部实测取证）；方案实施状态与后续计划以 `docs/2026-08-15-blf-mdf-conversion-master-plan.md`（总计划）为准——H1/H2/H4 已实施并验收，H2b/H2c 已否决，H7/H3/H5/H6 见总计划**
...
| # | 方案 | 状态（2026-08-15 更新，详见总计划文档） | 预期节省 | 质量影响 | 验证手段（见 §6） |
|---|---|---|---|---|---|
| H1 | BLF 解析 + 帧路由向量化（numpy 批量对象行走） | ✅ 已实施并验收（commit cfd6fc2；62.0s → 27.3s，输出逐位不变） | 读入 38.3s → 13.4s（总计划 H7 继续收口） | 零（逐位一致） | full_compare + compare_two_mdf + golden + 既有 82 项测试 |
| H2 | 桶存储 bytes → uint8 数组，并行解码 IPC 减负 | ✅ 主体已随 H1 落地（解码 10.5 → 1.5s）；H2a 已实施未提交；H2b/H2c 已否决 | 解码 10.5s → ~5s ✓ | 零 | 同上 + test_parallel_decode |
| H3 | 输入探测向量化 或 (路径,大小,mtime) 缓存 | ⬜ 待实施（总计划 §5.2） | 输入 8.55s → 预估 ~1.5s | 零 | probe vs list 对拍测试 |
| H4 | 统计聚合输入数组化（省 list→np.asarray 转换） | ✅ 已实施（随 H1；1.84 → 0.31s） | 1.84s → 0.31s ✓ | 零 | test_stats + full_compare 统计组 |
| H5 | 写 MDF 压缩级别 9→6（可选权衡） | ⬜ 待需求裁决（总计划 §5.3） | 8.2s → 预估 ~6s | **值不变、文件 +1-2%**（12.9MB → ~13.2MB，离 CANoe 10.3MB 更远） | full_compare（值逐点一致） |
| H6 | 进程池 spawn 预热重叠化（真正藏进扫描期或 lazy 化） | ◐ warm_up 全量预热已随 H1 提交；spawn 与读入重叠未做（总计划 §5.4） | ~1.7s | 零 | 计时复核 |
...
已实施并验收：H1（含 H2/H4 配套，commit cfd6fc2，62.0s → 27.3s，输出逐位不变）。本报告方案的盘点、决策记录与剩余计划（H7 读入残余 / H3 输入探测 / H5 压缩裁决 / H6 预热残余）统一见 `docs/2026-08-15-blf-mdf-conversion-master-plan.md`。
